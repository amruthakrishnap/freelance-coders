...
def scrape():
    ...
    # Fetch tweets asynchronously (assuming `fetch_tweets` is your function)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    extracted_data, cursor = loop.run_until_complete(fetch_tweets(rest_id, end_date))

    return jsonify(extracted_data)  # Send the tweet data back as JSON
